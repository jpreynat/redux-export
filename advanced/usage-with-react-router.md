...
`params` property is an object with every param specified in the url with the `match` object. _e.g: _`match.params`_ will be equal to _`{ filter: 'SHOW_COMPLETED' }`_ if we are navigating to _`localhost:3000/SHOW_COMPLETED`_. We can now read the URL from _`<App />`_._
