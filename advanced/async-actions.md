...
>
> Here, we assume that you understand reducer composition with [`combineReducers()`](../api-reference/combinereducers.md), as described in the [Splitting Reducers](../basics/reducers.md#splitting-reducers) section on the [basics guide](../basics/README.md). If you don't, please [read it first](../basics/reducers.md#splitting-reducers).
...
How do we include the Redux Thunk middleware in the dispatch mechanism? We use the [`applyMiddleware()`](../api-reference/applymiddleware.md) store enhancer from Redux, as shown below:
...
Dispatching async actions is no different from dispatching synchronous actions, so we won't discuss this in detail. See [Usage with React](../basics/usage-with-react.md) for an introduction into using Redux from React components. See [Example: Reddit API](example-reddit-api.md) for the complete source code discussed in this example.

## Next Steps

Read [Async Flow](async-flow.md) to recap how async actions fit into the Redux flow.
