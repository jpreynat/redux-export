...
* [Why should I use an immutable-focused library such as Immutable.JS?](using-immutable.js-with-redux.md#why-use-immutable-library)
* [Why should I choose Immutable.JS as an immutable library?](using-immutable.js-with-redux.md#why-choose-immutable-js)
* [What are the issues with using Immutable.JS?](using-immutable.js-with-redux.md#issues-with-immutable-js)
* [Is Immutable.JS worth the effort?](using-immutable.js-with-redux.md#is-immutable-js-worth-effort)
* [What are some opinionated Best Practices for using Immutable.JS with Redux?](using-immutable.js-with-redux.md#immutable-js-best-practices)
...
This can be prevented by using `toJS()` in a Higher Order Component, as discussed in the [Best Practices section](using-immutable.js-with-redux.md#immutable-js-best-practices) below.
...
* [FAQ: Immutable Data](https://github.com/jpreynat/redux/tree/f65d1ca135cfeb652b02ed39bfdb19c93e086ddc/docs/faq/ImmutableData.html#immutability-issues-with-react-redux)
...
_Note: if your app requires high performance, you may need to avoid _`toJS()`_ altogether, and so will have to use Immutable.JS in your dumb components. However, for most apps this will not be the case, and the benefits of keeping Immutable.JS out of your dumb components \(maintainability, portability and easier testing\) will far outweigh any perceived performance improvements of keeping it in._

_In addition, using _`toJS`_ in a Higher Order Component should not cause much, if any, performance degradation, as the component will only be called when the connected component’s props change. As with any performance issue, conduct performance checks first before deciding what to optimize._
