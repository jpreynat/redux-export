...
_Note: Reducers whose initial state is populated using _`preloadedState`_ will **still need to provide a default value** to handle when passed a _`state`_ of _`undefined`_. All reducers are passed _`undefined`_ on initialization, so they should be written such that when given _`undefined`_, some value should be returned. This can be any non-_`undefined`_ value; there's no need to duplicate the section of _`preloadedState`_ here as the default._
